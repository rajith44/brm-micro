...
export default function Header({ gemCategories = [], jewelryCategories = [] }) {
  ...
  return (
    <>
      ...
      {/* Mobile overlay — rendered outside header so backdrop-blur doesn't clip fixed positioning */}
      <div
        onClick={closeMobile}
        aria-hidden={!mobileOpen}
        className={`fixed inset-0 bg-black/40 transition-all duration-300 lg:hidden z-[100] ${mobileOpen ? "opacity-100 visible pointer-events-auto" : "opacity-0 invisible pointer-events-none"
          }`}
      />
      ...
      <aside
        aria-hidden={!mobileOpen}
        className={`fixed top-0 left-0 h-full w-[88%] max-w-[360px] bg-cream shadow-2xl transition-transform duration-300 lg:hidden z-[110] overflow-y-auto ${mobileOpen ? "translate-x-0" : "-translate-x-full pointer-events-none"
          }`}
      >
